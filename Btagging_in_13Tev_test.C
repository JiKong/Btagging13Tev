...
// global variable================================================
	...
	float N_gen_in_tbcone15=0;
	float N_gen_in_tqcone15=0;
	float N_gen_in_tqbarcone15=0;	
	float N_gen_out_tcone15=0;
	float N_gen_out_tbcone15=0;
	float N_gen_out_tqcone15=0;
	float N_gen_out_tqbarcone15=0;
	float N_rec_in_tcone15=0;
	float N_rec_in_tbcone15=0;
	float N_rec_in_tqcone15=0;
	float N_rec_in_tqbarcone15=0;	
	float N_rec_out_tcone15=0;
	float N_rec_out_tbcone15=0;
	float N_rec_out_tqcone15=0;
	float N_rec_out_tqbarcone15=0;

	float N_rec_in_tcone15_a_s1=0;
	float N_rec_in_tbcone15_a_s1=0;
	float N_rec_in_tqcone15_a_s1=0;
	float N_rec_in_tqbarcone15_a_s1=0;	
	float N_rec_out_tcone15_a_s1=0;
	float N_rec_out_tbcone15_a_s1=0;
	float N_rec_out_tqcone15_a_s1=0;
	float N_rec_out_tqbarcone15_a_s1=0;
	
	// after selection2 : PT>200
	float N_gen_in_tcone15_a_s2=0;
	float N_gen_in_tbcone15_a_s2=0;
	float N_gen_in_tqcone15_a_s2=0;
	float N_gen_in_tqbarcone15_a_s2=0;	
	float N_gen_out_tcone15_a_s2=0;
	float N_gen_out_tbcone15_a_s2=0;
	float N_gen_out_tqcone15_a_s2=0;
	float N_gen_out_tqbarcone15_a_s2=0;
	float N_rec_in_tcone15_a_s2=0;
	float N_rec_in_tbcone15_a_s2=0;
	float N_rec_in_tqcone15_a_s2=0;
	float N_rec_in_tqbarcone15_a_s2=0;	
	float N_rec_out_tcone15_a_s2=0;
	float N_rec_out_tbcone15_a_s2=0;
	float N_rec_out_tqcone15_a_s2=0;
	float N_rec_out_tqbarcone15_a_s2=0;
	
	float N_rec_in_tcone15_a_s12=0;
	float N_rec_in_tbcone15_a_s12=0;
	float N_rec_in_tqcone15_a_s12=0;
	float N_rec_in_tqbarcone15_a_s12=0;	
	float N_rec_out_tcone15_a_s12=0;
	float N_rec_out_tbcone15_a_s12=0;
	float N_rec_out_tqcone15_a_s12=0;
	float N_rec_out_tqbarcone15_a_s12=0;
...
void save_hist(TH1D* h_, TString dirname, TString filename) 
{
...
void save_hist(TH2D* h_, TString dirname, TString filename) 
{
...
// tcone15 test gen_par and THINjet seleciton
bool is_pass_s1(THINjet this_j)
{
	if (this_j.cisvv2<=1 && this_j.cisvv2>=0.605)
	{}
	else {return false;}
	if (this_j.p4.Pt()>=30 && abs(this_j.p4.Eta())<=2.5)
	{}
	else {return false;}
...
void Btagging_in_13Tev()
{
	...
	for (int sample_i=0;sample_i<1; sample_i++) //collect all small samples in a big sample
	{
		...
		TH1D* h_gen_par_id = new TH1D("h_gen_par_id","h_gen_par_id",60,-30,30);
		...
		TH1D* h_gen_tP4M = new TH1D("h_gen_tP4M","h_gen_tP4M",300,0,300);
		...
		TH1D* h_rec_tP4M = new TH1D("h_rec_tP4M","h_rec_tP4M",300,0,300);
		
		// rec level after selection1: 0.605<=THINjetCISVV2<=1, THINjet Pt >30, |THINjet Eta|<2.5
		TH1D* h_rec_dr_tP4b_a_s1 = new TH1D("h_rec_dr_tP4b_a_s1","h_rec_dr_tP4b_a_s1",700,0,7);
		TH1D* h_rec_dr_tP4q_a_s1 = new TH1D("h_rec_dr_tP4q_a_s1","h_rec_dr_tP4q_a_s1",700,0,7);
		TH1D* h_rec_dr_tP4qbar_a_s1 = new TH1D("h_rec_dr_tP4qbar_a_s1","h_rec_dr_tP4qbar_a_s1",700,0,7);
		TH1D* h_rec_tP4M_a_s1 = new TH1D("h_rec_tP4M_a_s1","h_rec_tP4M_a_s1",300,0,300);

		// after selection2:gen level top pt>200		
		TH1D* h_gen_dr_tP4b_a_s2 = new TH1D("h_gen_dr_tP4b_a_s2","h_gen_dr_tP4b_a_s2",700,0,7);
		TH1D* h_gen_dr_tP4q_a_s2 = new TH1D("h_gen_dr_tP4q_a_s2","h_gen_dr_tP4q_a_s2",700,0,7);
		TH1D* h_gen_dr_tP4qbar_a_s2 = new TH1D("h_gen_dr_tP4qbar_a_s2","h_gen_dr_tP4qbar_a_s2",700,0,7);
		TH1D* h_gen_tP4M_a_s2 = new TH1D("h_gen_tP4M_a_s2","h_gen_tP4M_a_s2",300,0,300);
		
		TH1D* h_rec_dr_tP4b_a_s2 = new TH1D("h_rec_dr_tP4b_a_s2","h_rec_dr_tP4b_a_s2",700,0,7);
		TH1D* h_rec_dr_tP4q_a_s2 = new TH1D("h_rec_dr_tP4q_a_s2","h_rec_dr_tP4q_a_s2",700,0,7);
		TH1D* h_rec_dr_tP4qbar_a_s2 = new TH1D("h_rec_dr_tP4qbar_a_s2","h_rec_dr_tP4qbar_a_s2",700,0,7);
		TH1D* h_rec_tP4M_a_s2 = new TH1D("h_rec_tP4M_a_s2","h_rec_tP4M_a_s2",300,0,300);	
		
		TH1D* h_rec_dr_tP4b_a_s12 = new TH1D("h_rec_dr_tP4b_a_s12","h_rec_dr_tP4b_a_s12",700,0,7);
		TH1D* h_rec_dr_tP4q_a_s12 = new TH1D("h_rec_dr_tP4q_a_s12","h_rec_dr_tP4q_a_s12",700,0,7);
		TH1D* h_rec_dr_tP4qbar_a_s12 = new TH1D("h_rec_dr_tP4qbar_a_s12","h_rec_dr_tP4qbar_a_s12",700,0,7);
		TH1D* h_rec_tP4M_a_s12 = new TH1D("h_rec_tP4M_a_s12","h_rec_tP4M_a_s12",300,0,300);	
	
		// gen: top PT v.s deltaR(b,l)
		TH2D* h_gen_tpt_drbl = new TH2D("gen_tpt_drbl", "gen_tpt_drbl", 70, 0, 7 , 50 ,0 ,500 );
			...
			*/
			...
			for(int i=0;i<nGenPar;i++)  //gen par loop for TTbar sample
			{
				...
				)
				{
					...
					//test
					if ((genParId[i]==13 || genParId[i]==11) && genMomParId[i]==-24){ttt1++;}
					if ((genParId[i]==13 || genParId[i]==11) && genMomParId[i]==24){ttt2++;}
				}
			...
			int gen_lplus_from_wplus_index=-1;
			int gen_lminus_from_wminus_index=-1;
			...
			int rec_b_index_a_s1=-1;
			int rec_bbar_index_a_s1=-1;
			int rec_q_from_wplus_index_a_s1=-1;
			int rec_qbar_from_wplus_index_a_s1=-1;
			int rec_q_from_wminus_index_a_s1=-1;
			int rec_qbar_from_wminus_index_a_s1=-1;
			...
			for (int i=0;i< this_ev.gen_pars.size();i++)
			{
				...
				else if ((this_g.id==11 || this_g.id==13) && abs(this_g.mid) == 24){gen_lplus_from_wplus_index=i;} //l+ from W+
				else if ((this_g.id==-11 || this_g.id==-13) && abs(this_g.mid) == -24){gen_lminus_from_wminus_index=i;} //l- from W-				
			}
			...
			for (int i=0;i<this_ev.THINjets.size();i++)
			{
				...
				{ 
					...
					if (is_pass_s1(this_j)){rec_qbar_from_wminus_index_a_s1=i;}
				}			
			}
			...
			if ( gen_b_index!=-1 && gen_q_from_wplus_index!=-1 && gen_qbar_from_wplus_index!=-1)
			{
				...
				h_gen_tP4M->Fill(tl.M());
				
				if (gen_lplus_from_wplus_index!=-1)
				{
					TLorentzVector ll =this_ev.gen_pars[gen_lplus_from_wplus_index].p4;
					h_gen_tpt_drbl->Fill(delta_R(tl,ll), tl.Pt() );
				}
				
				if (delta_R(tl, bl )<=1.5)
				{ N_gen_in_tbcone15++;}
				else
				{ N_gen_out_tbcone15++;}
				if (delta_R(tl, ql )<=1.5)
				{ N_gen_in_tqcone15++;}
				else
				{ N_gen_out_tqcone15++;}
				if (delta_R(tl, qbarl )<=1.5)
				{ N_gen_in_tqbarcone15++;}
				else
				{ N_gen_out_tqbarcone15++;}
				...
				if (tl.Pt()>=200)
				{
					h_gen_dr_tP4b_a_s2->Fill(delta_R(tl, bl ));
					h_gen_dr_tP4q_a_s2->Fill(delta_R(tl, ql ));
					h_gen_dr_tP4qbar_a_s2->Fill(delta_R(tl, qbarl ));
					h_gen_tP4M_a_s2->Fill(tl.M());
					
					if (delta_R(tl, bl )<=1.5)
					{ N_gen_in_tbcone15_a_s2++;}
					else
					{ N_gen_out_tbcone15_a_s2++;}
					if (delta_R(tl, ql )<=1.5)
					{ N_gen_in_tqcone15_a_s2++;}
					else
					{ N_gen_out_tqcone15_a_s2++;}
					if (delta_R(tl, qbarl )<=1.5)
					{ N_gen_in_tqbarcone15_a_s2++;}
					else
					{ N_gen_out_tqbarcone15_a_s2++;}
					if (delta_R(tl, bl )<=1.5 && delta_R(tl, ql )<1.5 && delta_R(tl, qbarl )<1.5)
					{ N_gen_in_tcone15_a_s2++;}
					else 
					{N_gen_out_tcone15_a_s2++;}
			...
			{N_gen_good_t++;
				...
				if (gen_lminus_from_wminus_index!=-1)
				{
					TLorentzVector ll =this_ev.gen_pars[gen_lminus_from_wminus_index].p4;
					h_gen_tpt_drbl->Fill(delta_R(tl,ll), tl.Pt() );
				}
			...
			// rec leval (before any cut)h_dr_tP4b, h_dr_tP4q,  h_dr_tP4qbar
			if ( rec_b_index!=-1 && rec_q_from_wplus_index!=-1 && rec_qbar_from_wplus_index!=-1)
			{
				...
				{ N_rec_out_tqcone15++;}
				if (delta_R(tl, qbarl )<=1.5)
				{ N_rec_in_tqbarcone15++;}
				else
				{ N_rec_out_tqbarcone15++;}
				if (delta_R(tl, bl )<=1.5 && delta_R(tl, ql )<1.5 && delta_R(tl, qbarl )<1.5)
				{ N_rec_in_tcone15++;}
				else 
				{N_rec_out_tcone15++;}
				...
				if (tl.Pt()>=200)
				{
					h_rec_dr_tP4b_a_s2->Fill(delta_R(tl, bl ));
					h_rec_dr_tP4q_a_s2->Fill(delta_R(tl, ql ));
					h_rec_dr_tP4qbar_a_s2->Fill(delta_R(tl, qbarl ));
					h_rec_tP4M_a_s2->Fill(tl.M());
					
					if (delta_R(tl, bl )<=1.5)
					{ N_rec_in_tbcone15_a_s2++;}
					else
					{ N_rec_out_tbcone15_a_s2++;}
					if (delta_R(tl, ql )<=1.5)
					{ N_rec_in_tqcone15_a_s2++;}
					else
					{ N_rec_out_tqcone15_a_s2++;}
					if (delta_R(tl, qbarl )<=1.5)
					{ N_rec_in_tqbarcone15_a_s2++;}
					else
					{ N_rec_out_tqbarcone15_a_s2++;}
					if (delta_R(tl, bl )<=1.5 && delta_R(tl, ql )<1.5 && delta_R(tl, qbarl )<1.5)
					{ N_rec_in_tcone15_a_s2++;}
					else 
					{N_rec_out_tcone15_a_s2++;}
			...
			// rec leval (after s1) h_dr_tP4b, h_dr_tP4q,  h_dr_tP4qbar
			if ( rec_b_index_a_s1!=-1 && rec_q_from_wplus_index_a_s1!=-1 && rec_qbar_from_wplus_index_a_s1!=-1)
			{
				TLorentzVector bl =this_ev.THINjets[rec_b_index_a_s1].p4;
				TLorentzVector ql =this_ev.THINjets[rec_q_from_wplus_index_a_s1].p4;
				TLorentzVector qbarl =this_ev.THINjets[rec_qbar_from_wplus_index_a_s1].p4;
				TLorentzVector tl = bl+ql+qbarl;
				h_rec_dr_tP4b_a_s1->Fill(delta_R(tl, bl ));
				h_rec_dr_tP4q_a_s1->Fill(delta_R(tl, ql ));
				h_rec_dr_tP4qbar_a_s1->Fill(delta_R(tl, qbarl ));
				h_rec_tP4M_a_s1->Fill(tl.M());
				
				if (delta_R(tl, bl )<=1.5)
				{ N_rec_in_tbcone15_a_s1++;}
				else
				{ N_rec_out_tbcone15_a_s1++;}
				if (delta_R(tl, ql )<=1.5)
				{ N_rec_in_tqcone15_a_s1++;}
				else
				{ N_rec_out_tqcone15_a_s1++;}
				if (delta_R(tl, qbarl )<=1.5)
				{ N_rec_in_tqbarcone15_a_s1++;}
				else
				{ N_rec_out_tqbarcone15_a_s1++;}
				if (delta_R(tl, bl )<=1.5 && delta_R(tl, ql )<1.5 && delta_R(tl, qbarl )<1.5)
				{ N_rec_in_tcone15_a_s1++;}
				else 
				{N_rec_out_tcone15_a_s1++;}
				
				if (tl.Pt()>=200)
				{
					h_rec_dr_tP4b_a_s12->Fill(delta_R(tl, bl ));
					h_rec_dr_tP4q_a_s12->Fill(delta_R(tl, ql ));
					h_rec_dr_tP4qbar_a_s12->Fill(delta_R(tl, qbarl ));
					h_rec_tP4M_a_s12->Fill(tl.M());
					
					if (delta_R(tl, bl )<=1.5)
					{ N_rec_in_tbcone15_a_s12++;}
					else
					{ N_rec_out_tbcone15_a_s12++;}
					if (delta_R(tl, ql )<=1.5)
					{ N_rec_in_tqcone15_a_s12++;}
					else
					{ N_rec_out_tqcone15_a_s12++;}
					if (delta_R(tl, qbarl )<=1.5)
					{ N_rec_in_tqbarcone15_a_s12++;}
					else
					{ N_rec_out_tqbarcone15_a_s12++;}
					if (delta_R(tl, bl )<=1.5 && delta_R(tl, ql )<1.5 && delta_R(tl, qbarl )<1.5)
					{ N_rec_in_tcone15_a_s12++;}
					else 
					{N_rec_out_tcone15_a_s12++;}
				}
			}
			if ( rec_bbar_index_a_s1!=-1 && rec_q_from_wminus_index_a_s1!=-1 && rec_qbar_from_wminus_index_a_s1!=-1)
			{
				TLorentzVector bl =this_ev.THINjets[rec_bbar_index_a_s1].p4;
				TLorentzVector ql =this_ev.THINjets[rec_q_from_wminus_index_a_s1].p4;
				TLorentzVector qbarl =this_ev.THINjets[rec_qbar_from_wminus_index_a_s1].p4;
				TLorentzVector tl = bl+ql+qbarl;
				h_rec_dr_tP4b_a_s1->Fill(delta_R(tl, bl ));
				h_rec_dr_tP4q_a_s1->Fill(delta_R(tl, ql ));
				h_rec_dr_tP4qbar_a_s1->Fill(delta_R(tl, qbarl ));
		...
		*/
		...
		// event loop : counting b-tagging for MC
	  	for (Long64_t Nth = 0; Nth < Nev_MC_pass_gen_selection; Nth++)
			...
			{
				h_gen_par_id->Fill(events_MC[Nth].gen_pars[i].id);
...
*/
		...
		cout<<"t & b:"<<"N_gen_in_tbcone15/(N_gen_in_tbcone15+N_gen_out_tbcone15):"<<N_gen_in_tbcone15<<"/"<<(N_gen_in_tbcone15+N_gen_out_tbcone15)<<"="<<N_gen_in_tbcone15/(N_gen_in_tbcone15+N_gen_out_tbcone15)<<endl;
		cout<<"t & q:"<<"N_gen_in_tqcone15/(N_gen_in_tqcone15+N_gen_out_tqcone15):"<<N_gen_in_tqcone15<<"/"<<(N_gen_in_tqcone15+N_gen_out_tqcone15)<<"="<<N_gen_in_tqcone15/(N_gen_in_tqcone15+N_gen_out_tqcone15)<<endl;
		cout<<"t & qbar:"<<"N_gen_in_tqbarcone15/(N_gen_in_tqbarcone15+N_gen_out_tqbarcone15):"<<N_gen_in_tqbarcone15<<"/"<<(N_gen_in_tqbarcone15+N_gen_out_tqbarcone15)<<"="<<N_gen_in_tqbarcone15/(N_gen_in_tqbarcone15+N_gen_out_tqbarcone15)<<endl;
		cout<<"t & all:"<<"N_gen_in_tcone15/(N_gen_in_tcone15+N_gen_out_tcone15):"<<N_gen_in_tcone15<<"/"<<(N_gen_in_tcone15+N_gen_out_tcone15)<<"="<<N_gen_in_tcone15/(N_gen_in_tcone15+N_gen_out_tcone15)<<endl;
		cout<<endl;

		cout<<"t & b:"<<"N_rec_in_tbcone15/(N_rec_in_tbcone15+N_rec_out_tbcone15):"<<N_rec_in_tbcone15<<"/"<<(N_rec_in_tbcone15+N_rec_out_tbcone15)<<"="<<N_rec_in_tbcone15/(N_rec_in_tbcone15+N_rec_out_tbcone15)<<endl;
		cout<<"t & q:"<<"N_rec_in_tqcone15/(N_rec_in_tqcone15+N_rec_out_tqcone15):"<<N_rec_in_tqcone15<<"/"<<(N_rec_in_tqcone15+N_rec_out_tqcone15)<<"="<<N_rec_in_tqcone15/(N_rec_in_tqcone15+N_rec_out_tqcone15)<<endl;
		cout<<"t & qbar:"<<"N_rec_in_tqbarcone15/(N_rec_in_tqbarcone15+N_rec_out_tqbarcone15):"<<N_rec_in_tqbarcone15<<"/"<<(N_rec_in_tqbarcone15+N_rec_out_tqbarcone15)<<"="<<N_rec_in_tqbarcone15/(N_rec_in_tqbarcone15+N_rec_out_tqbarcone15)<<endl;
		cout<<"t & all:"<<"N_rec_in_tcone15/(N_rec_in_tcone15+N_rec_out_tcone15):"<<N_rec_in_tcone15<<"/"<<(N_rec_in_tcone15+N_rec_out_tcone15)<<"="<<N_rec_in_tcone15/(N_rec_in_tcone15+N_rec_out_tcone15)<<endl;
		cout<<endl;	

		cout<<"t & b:"<<"N_rec_in_tbcone15_a_s1/(N_rec_in_tbcone15_a_s1+N_rec_out_tbcone15_a_s1):"<<N_rec_in_tbcone15_a_s1<<"/"<<(N_rec_in_tbcone15_a_s1+N_rec_out_tbcone15_a_s1)<<"="<<N_rec_in_tbcone15_a_s1/(N_rec_in_tbcone15_a_s1+N_rec_out_tbcone15_a_s1)<<endl;
		cout<<"t & q:"<<"N_rec_in_tqcone15_a_s1/(N_rec_in_tqcone15_a_s1+N_rec_out_tqcone15_a_s1):"<<N_rec_in_tqcone15_a_s1<<"/"<<(N_rec_in_tqcone15_a_s1+N_rec_out_tqcone15_a_s1)<<"="<<N_rec_in_tqcone15_a_s1/(N_rec_in_tqcone15_a_s1+N_rec_out_tqcone15_a_s1)<<endl;
		cout<<"t & qbar:"<<"N_rec_in_tqbarcone15_a_s1/(N_rec_in_tqbarcone15_a_s1+N_rec_out_tqbarcone15_a_s1):"<<N_rec_in_tqbarcone15_a_s1<<"/"<<(N_rec_in_tqbarcone15_a_s1+N_rec_out_tqbarcone15_a_s1)<<"="<<N_rec_in_tqbarcone15_a_s1/(N_rec_in_tqbarcone15_a_s1+N_rec_out_tqbarcone15_a_s1)<<endl;
		cout<<"t & all:"<<"N_rec_in_tcone15_a_s1/(N_rec_in_tcone15_a_s1+N_rec_out_tcone15_a_s1):"<<N_rec_in_tcone15_a_s1<<"/"<<(N_rec_in_tcone15_a_s1+N_rec_out_tcone15_a_s1)<<"="<<N_rec_in_tcone15_a_s1/(N_rec_in_tcone15_a_s1+N_rec_out_tcone15_a_s1)<<endl;
		cout<<endl;		

		cout<<"t & b:"<<"N_gen_in_tbcone15_a_s2/(N_gen_in_tbcone15_a_s2+N_gen_out_tbcone15_a_s2):"<<N_gen_in_tbcone15_a_s2<<"/"<<(N_gen_in_tbcone15_a_s2+N_gen_out_tbcone15_a_s2)<<"="<<N_gen_in_tbcone15_a_s2/(N_gen_in_tbcone15_a_s2+N_gen_out_tbcone15_a_s2)<<endl;
		cout<<"t & q:"<<"N_gen_in_tqcone15_a_s2/(N_gen_in_tqcone15_a_s2+N_gen_out_tqcone15_a_s2):"<<N_gen_in_tqcone15_a_s2<<"/"<<(N_gen_in_tqcone15_a_s2+N_gen_out_tqcone15_a_s2)<<"="<<N_gen_in_tqcone15_a_s2/(N_gen_in_tqcone15_a_s2+N_gen_out_tqcone15_a_s2)<<endl;
		cout<<"t & qbar:"<<"N_gen_in_tqbarcone15_a_s2/(N_gen_in_tqbarcone15_a_s2+N_gen_out_tqbarcone15_a_s2):"<<N_gen_in_tqbarcone15_a_s2<<"/"<<(N_gen_in_tqbarcone15_a_s2+N_gen_out_tqbarcone15_a_s2)<<"="<<N_gen_in_tqbarcone15_a_s2/(N_gen_in_tqbarcone15_a_s2+N_gen_out_tqbarcone15_a_s2)<<endl;
		cout<<"t & all:"<<"N_gen_in_tcone15_a_s2/(N_gen_in_tcone15_a_s2+N_gen_out_tcone15_a_s2):"<<N_gen_in_tcone15_a_s2<<"/"<<(N_gen_in_tcone15_a_s2+N_gen_out_tcone15_a_s2)<<"="<<N_gen_in_tcone15_a_s2/(N_gen_in_tcone15_a_s2+N_gen_out_tcone15_a_s2)<<endl;
		cout<<endl;

		cout<<"t & b:"<<"N_rec_in_tbcone15_a_s2/(N_rec_in_tbcone15_a_s2+N_rec_out_tbcone15_a_s2):"<<N_rec_in_tbcone15_a_s2<<"/"<<(N_rec_in_tbcone15_a_s2+N_rec_out_tbcone15_a_s2)<<"="<<N_rec_in_tbcone15_a_s2/(N_rec_in_tbcone15_a_s2+N_rec_out_tbcone15_a_s2)<<endl;
		cout<<"t & q:"<<"N_rec_in_tqcone15_a_s2/(N_rec_in_tqcone15_a_s2+N_rec_out_tqcone15_a_s2):"<<N_rec_in_tqcone15_a_s2<<"/"<<(N_rec_in_tqcone15_a_s2+N_rec_out_tqcone15_a_s2)<<"="<<N_rec_in_tqcone15_a_s2/(N_rec_in_tqcone15_a_s2+N_rec_out_tqcone15_a_s2)<<endl;
		cout<<"t & qbar:"<<"N_rec_in_tqbarcone15_a_s2/(N_rec_in_tqbarcone15_a_s2+N_rec_out_tqbarcone15_a_s2):"<<N_rec_in_tqbarcone15_a_s2<<"/"<<(N_rec_in_tqbarcone15_a_s2+N_rec_out_tqbarcone15_a_s2)<<"="<<N_rec_in_tqbarcone15_a_s2/(N_rec_in_tqbarcone15_a_s2+N_rec_out_tqbarcone15_a_s2)<<endl;
		cout<<"t & all:"<<"N_rec_in_tcone15_a_s2/(N_rec_in_tcone15_a_s2+N_rec_out_tcone15_a_s2):"<<N_rec_in_tcone15_a_s2<<"/"<<(N_rec_in_tcone15_a_s2+N_rec_out_tcone15_a_s2)<<"="<<N_rec_in_tcone15_a_s2/(N_rec_in_tcone15_a_s2+N_rec_out_tcone15_a_s2)<<endl;
		cout<<endl;
		
		cout<<"t & b:"<<"N_rec_in_tbcone15_a_s12/(N_rec_in_tbcone15_a_s12+N_rec_out_tbcone15_a_s12):"<<N_rec_in_tbcone15_a_s12<<"/"<<(N_rec_in_tbcone15_a_s12+N_rec_out_tbcone15_a_s12)<<"="<<N_rec_in_tbcone15_a_s12/(N_rec_in_tbcone15_a_s12+N_rec_out_tbcone15_a_s12)<<endl;
		cout<<"t & q:"<<"N_rec_in_tqcone15_a_s12/(N_rec_in_tqcone15_a_s12+N_rec_out_tqcone15_a_s12):"<<N_rec_in_tqcone15_a_s12<<"/"<<(N_rec_in_tqcone15_a_s12+N_rec_out_tqcone15_a_s12)<<"="<<N_rec_in_tqcone15_a_s12/(N_rec_in_tqcone15_a_s12+N_rec_out_tqcone15_a_s12)<<endl;
		cout<<"t & qbar:"<<"N_rec_in_tqbarcone15_a_s12/(N_rec_in_tqbarcone15_a_s12+N_rec_out_tqbarcone15_a_s12):"<<N_rec_in_tqbarcone15_a_s12<<"/"<<(N_rec_in_tqbarcone15_a_s12+N_rec_out_tqbarcone15_a_s12)<<"="<<N_rec_in_tqbarcone15_a_s12/(N_rec_in_tqbarcone15_a_s12+N_rec_out_tqbarcone15_a_s12)<<endl;
		cout<<"t & all:"<<"N_rec_in_tcone15_a_s12/(N_rec_in_tcone15_a_s12+N_rec_out_tcone15_a_s12):"<<N_rec_in_tcone15_a_s12<<"/"<<(N_rec_in_tcone15_a_s12+N_rec_out_tcone15_a_s12)<<"="<<N_rec_in_tcone15_a_s12/(N_rec_in_tcone15_a_s12+N_rec_out_tcone15_a_s12)<<endl;
		cout<<endl;	
...
*/
	...
	*/
	...
	save_hist(h_gen_par_id, "pic_13Tev_TTbar", "gen_par_id");

	// gen level
	//save_hist(h_gen_dr_tb, "pic_13Tev_TTbar", "h_gen_dr_tb");
	//save_hist(h_gen_dr_tq, "pic_13Tev_TTbar", "h_gen_dr_tq");
	//save_hist(h_gen_dr_tqbar, "pic_13Tev_TTbar", "h_gen_dr_tqbar");
	//save_hist(h_gen_tP4M, "pic_13Tev_TTbar", "h_gen_tP4M");
	
	// rec level and gen level P4 comparison
	...
	save_overlap_hist(h_gen_tP4M, h_rec_tP4M,"pic_13Tev_TTbar", "h_tP4M");
	
	// rec level after selection1: 0.605<=THINjetCISVV2<=1, THINjet Pt >30, |THINjet Eta|<2.5
	save_hist(h_rec_dr_tP4b_a_s1, "pic_13Tev_TTbar", "h_rec_dr_tP4b_a_s1");
	save_hist(h_rec_dr_tP4q_a_s1, "pic_13Tev_TTbar", "h_rec_dr_tP4q_a_s1");
	save_hist(h_rec_dr_tP4qbar_a_s1, "pic_13Tev_TTbar", "h_rec_dr_tP4qbar_a_s1");
	save_hist(h_rec_tP4M_a_s1, "pic_13Tev_TTbar", "h_rec_tP4M_a_s1");
	
	// after selection2:gen level top pt>200
	save_hist(h_gen_dr_tP4b_a_s2, "pic_13Tev_TTbar", "h_gen_dr_tP4b_a_s2");
	save_hist(h_gen_dr_tP4q_a_s2, "pic_13Tev_TTbar", "h_gen_dr_tP4q_a_s2");
	save_hist(h_gen_dr_tP4qbar_a_s2, "pic_13Tev_TTbar", "h_gen_dr_tP4qbar_a_s2");
	save_hist(h_gen_tP4M_a_s2 , "pic_13Tev_TTbar", "h_gen_tP4M_a_s2");
	
	save_hist(h_rec_dr_tP4b_a_s2, "pic_13Tev_TTbar", "h_rec_dr_tP4b_a_s2");
	save_hist(h_rec_dr_tP4q_a_s2, "pic_13Tev_TTbar", "h_rec_dr_tP4q_a_s2");
	save_hist(h_rec_dr_tP4qbar_a_s2, "pic_13Tev_TTbar", "h_rec_dr_tP4qbar_a_s2");
	save_hist(h_rec_tP4M_a_s2 , "pic_13Tev_TTbar", "h_rec_tP4M_a_s2");
	
	save_hist(h_rec_dr_tP4b_a_s12, "pic_13Tev_TTbar", "h_rec_dr_tP4b_a_s12");
	save_hist(h_rec_dr_tP4q_a_s12, "pic_13Tev_TTbar", "h_rec_dr_tP4q_a_s12");
	save_hist(h_rec_dr_tP4qbar_a_s12, "pic_13Tev_TTbar", "h_rec_dr_tP4qbar_a_s12");
	save_hist(h_rec_tP4M_a_s12 , "pic_13Tev_TTbar", "h_rec_tP4M_a_s12");	
	
	// gen: top PT v.s deltaR(b,l)
	save_hist(h_gen_tpt_drbl, "pic_13Tev_TTbar", "h_gen_tpt_drbl");
