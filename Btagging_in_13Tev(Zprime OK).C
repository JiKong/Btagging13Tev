...
// global variable================================================
	...
	// counting
	int N_FATjet_before_deoverlap=0;
	int N_FATjet_after_deoverlap=0;
...
class FATjet : public jet
{
	...
	FATjet():jet(){}	
    	FATjet(TLorentzVector p4_,
		Float_t csv_,Float_t cisvv2_, Float_t pruned_mass_, Float_t sd_mass_, /* Int_t pass_id_,*/ Float_t tau1_, Float_t tau2_, bool is_tight_ ,bool is_loose_ /*, 
		Float_t n_had_ef_,Float_t n_em_ef_,Float_t c_had_ef_,Float_t c_em_ef_,Float_t c_multi_*/)
		:jet(p4_,
		csv_ ,cisvv2_ /*, 
		n_had_ef_, n_em_ef_, c_had_ef_, c_em_ef_, c_multi_*/)
    	{  pruned_mass = pruned_mass_;  sd_mass=sd_mass_; /*pass_id=pass_id_;*/ tau1=tau1_; tau2=tau2_; is_tight=is_tight_; is_loose=is_loose_; }
};
...
class event
{
	...
	vector< vector<FATsubjet> > FATsubjets;
	vector<FATjet> FATjets_deoverlap_with_lepton;
	vector<FATjet> good_FATjets;
	vector<ele> eles;
...
void save_hist(TH1D* h_, TString dirname, TString filename) // type: eff mr
{
	...
	TImage *img1 = TImage::Create();
	img1->FromPad(c1);
	img1->WriteImage("./Zprime_sample/"+dirname+"/"+filename+".png");
	delete c1;
	delete img1;
...
bool is_pass_FATjet_loose_id(FATjet j_)
{
...
{
return true; //tmp test
/*
	double iso_cut = 2+(0.03*e_.p4.Pt());
	
	if( e_.pass_ID <= 0 ) return false;
	//if( e_.p4.Pt() <= 40 ) return false;
	if( e_.p4.Pt() <= 10 ) return false;
	if( e_.user_trk_iso >= 5 ) return false;
	
	// barrel 
	if( abs(e_.p4.Eta()) > 0 && abs(e_.p4.Eta()) < 1.4442 )
	{ if( e_.user_cal_iso-0.06205*e_.rho >= iso_cut ){return false;} }
	
	// endcap selection
	if( abs(e_.p4.Eta()) > 1.566 && abs(e_.p4.Eta()) < 2.5 )
	{
		if( e_.p4.Pt() < 50 )
...
bool is_pass_mu_iso(mu m_)
{
	...
	/*
	if(m_.p4.Pt()<10){return false;}
	if(abs(m_.p4.Eta())>2.1){return false;}
	if(m_.corr_pf_iso/m_.p4.Pt()>0.05){return false;} */
...
bool is_pass_ev_selection_ttbar_semileptonic(event ev)
{
	...
	{
		mu this_mu=ev.mus[i];
		if(this_mu.p4.Pt()>35 && abs(this_mu.p4.Eta()<2.1) && is_pass_mu_loose_id(this_mu)){have_good_l=true; break;}
	}
	for(int i=0;i<ev.eles.size();i++)
	{
		ele this_ele=ev.eles[i];
		if(this_ele.p4.Pt()>35 && abs(this_ele.p4.Eta()<2.4) && is_pass_ele_loose_id(this_ele)){have_good_l=true; break;}
	}
	if(!have_good_l){ev_flag=2;}

	// 3: to check if have at least 4 good jet
	// using FATjet
	vector<FATjet> good_jets;
	for (int i=0;i<ev.FATjets.size();i++)
	{
	
		FATjet this_j = ev.FATjets[i];
		
		bool overlap_with_lepton=false;
		for (int elei=0;elei<ev.eles.size();elei++)
		{	
			ele this_ele = ev.eles[elei];
			if (!is_pass_ele_loose_id(this_ele) ){continue;}
...
//test
	...
	ev_flag:
		...
		=1: !(the deltaR between any pairs of FATsubjet in this event should be > 0.3), skip this event.
...
//evse
bool is_pass_ev_selection_Zprime_signal(event ev)
{
	int channel=0; //1 for ele, 2 for mu
	//to calculate # of "cadidate FATsub bjet" and # of "cadidate FATsub nbjet" in this event
	int ev_flag=0;
	
	// 1: event must have 2 good ele or 2 good muon
	...
	if (good_eles.size()>=2 && good_mus.size()<2){channel=1;}
	else if (good_eles.size()<2 && good_mus.size()>=2){channel=2;}
	else {ev_flag=1; return false;}

	
	// 2: recostructed Z pass mass window cut:
	
	TLorentzVector Z_lv;
	Z_lv.SetPtEtaPhiE(0,0,0,0);  
	
	TLorentzVector Z_from_2ele_lv;
	Z_from_2ele_lv.SetPtEtaPhiE(0,0,0,0);  
	
	TLorentzVector Z_from_2mu_lv;
	Z_from_2mu_lv.SetPtEtaPhiE(0,0,0,0);

	//if (good_eles.size()>=2)
	if (channel==1)
	{
		int good_ele1_index=-1;
		double good_ele1_pt=-1;
		int good_ele2_index=-1;
		double good_ele2_pt=-1;

		for(int i=0;i<good_eles.size();i++)
		{
			ele p = good_eles[i]; 
			if(p.p4.Pt()>=good_ele1_pt)
			{ good_ele1_index=i; good_ele1_pt=p.p4.Pt();}
		}
		for(int i=0;i<good_eles.size();i++)
		{
			if (i==good_ele1_index){continue;}
			ele p = good_eles[i]; 
			if(p.p4.Pt()>=good_ele2_pt)
			{ good_ele2_index=i; good_ele2_pt=p.p4.Pt();}
		}

		TLorentzVector good_ele1_lv=good_eles[good_ele1_index].p4;
		TLorentzVector good_ele2_lv=good_eles[good_ele2_index].p4;		

			
		Z_from_2ele_lv = good_ele1_lv+good_ele2_lv;
	}
	
	//if (good_mus.size()>=2)
	if (channel==2)
	{
		int good_mu1_index=-1;
		double good_mu1_pt=-1;
		int good_mu2_index=-1;
		double good_mu2_pt=-1;

		for(int i=0;i<good_mus.size();i++)
		{
			mu p = good_mus[i]; 
			if(p.p4.Pt()>=good_mu1_pt)
			{ good_mu1_index=i; good_mu1_pt=p.p4.Pt();}
		}
		for(int i=0;i<good_mus.size();i++)
		{
			if (i==good_mu1_index){continue;}
			mu p = good_mus[i]; 
			if(p.p4.Pt()>=good_mu2_pt)
			{ good_mu2_index=i; good_mu2_pt=p.p4.Pt();}
		}

		TLorentzVector good_mu1_lv=good_mus[good_mu1_index].p4;
		TLorentzVector good_mu2_lv=good_mus[good_mu2_index].p4;		
			
		Z_from_2mu_lv = good_mu1_lv+good_mu2_lv;


	}
	
	//if (Z_from_2ele_lv.Pt() > Z_from_2mu_lv.Pt())
	if (channel==1)
	{ Z_lv=Z_from_2ele_lv;}
	else if (channel==2)
	{ Z_lv=Z_from_2mu_lv;}
	if (Z_lv.M()<70 || Z_lv.M()>110){ev_flag=2; return false;}
	
	//cout<< "("<<Z_lv.Pt()<<","<<Z_lv.Eta()<<","<<Z_lv.Phi()<<","<<Z_lv.E() <<")"<<Z_lv.M()<<endl;

	// 3 recZ Pt cut
	if (Z_lv.Pt()<80){ev_flag=3; return false;}
	
	//  4: to check if have at least 1 good jet (using FATjet)

	for (int i=0;i<ev.FATjets.size();i++)
	{
		FATjet this_j = ev.FATjets[i];
		
		if(!(this_j.p4.Pt()>30 && abs(this_j.p4.Eta()<2.5) /*&& this_j.pass_id>0*/ && this_j.sd_mass>95 && this_j.sd_mass<145 &&  this_j.tau2/this_j.tau1<0.5 &&  is_pass_FATjet_loose_id(this_j)))
		{continue;}
		...
		ev.good_FATjets.push_back(this_j);
		
	}
	
	
	//if (ev.good_FATjets.size()<2){ ev_flag=4; return false;}
	if (ev.good_FATjets.size()<1){ ev_flag=4; return false;}
	
	//5:
	 
	//if(ev.good_FATjets[0].sd_mass>145 || ev.good_FATjets[0].sd_mass<95){ev_flag=5; return false;}
	//if(ev.good_FATjets[0].pruned_mass>120 || ev.good_FATjets[0].pruned_mass<80){ev_flag=5; return false;}
	//if (delta_R(ev.good_FATjets[0],ev.good_FATjets[1])<0.8){ev_flag=5; return false;}

	//if ((ev.good_FATjets[0].pruned_mass<145 && ev.good_FATjets[0].pruned_mass>105)||(ev.good_FATjets[1].pruned_mass<145 && ev.good_FATjets[1].pruned_mass>105)) {return true;}
	
	
	if (ev_flag!=0){return false;}
	return true;
...
void Btagging_in_13Tev()
{
	...
	//samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-600_13TeV-madgraph.root");
	//samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-800_13TeV-madgraph.root");
	samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-1000_13TeV-madgraph.root");
	samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-1200_13TeV-madgraph.root");
	samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-1400_13TeV-madgraph.root");
	samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-1600_13TeV-madgraph.root");
	samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-1800_13TeV-madgraph.root");
	samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-2000_13TeV-madgraph.root");
	samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-2500_13TeV-madgraph.root");
	samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-3000_13TeV-madgraph.root");
	samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-3500_13TeV-madgraph.root");
	samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-4000_13TeV-madgraph.root");
	samples.push_back("../13Tev/ZprimeToZhToZlephbb_narrow_M-4500_13TeV-madgraph.root");
	
	
	vector<TString> zpmass;
	//zpmass.push_back("_600");
	//zpmass.push_back("_800");
	zpmass.push_back("_1000");
	zpmass.push_back("_1200");
	zpmass.push_back("_1400");
	zpmass.push_back("_1600");
	zpmass.push_back("_1800");
	zpmass.push_back("_2000");
	zpmass.push_back("_2500");
	zpmass.push_back("_3000");
	zpmass.push_back("_3500");
	zpmass.push_back("_4000");
	zpmass.push_back("_4500");
	...
	for (int sample_i=0;sample_i<samples.size(); sample_i++)
	{
		...
		//histogram set
		TH1D* FATCSV_b_evcut = new TH1D("FATCSV_b_evcut"+zpmass[sample_i],"FATCSV_b_evcut"+zpmass[sample_i],100,0,1);
		TH1D* FATCSV_a_evcut = new TH1D("FATCSV_a_evcut"+zpmass[sample_i],"FATCSV_a_evcut"+zpmass[sample_i],100,0,1);
	
		TH1D* genbb_deltaR_b_evcut = new TH1D("genbb_deltaR_b_evcut"+zpmass[sample_i],"genbb_deltaR_b_evcut"+zpmass[sample_i],100,0,3);
		TH1D* leading_pruned_jet_mass_b_evcut = new TH1D("leading_pruned_jet_mass_b_evcut"+zpmass[sample_i],"leading_pruned_jet_mass_b_evcut"+zpmass[sample_i],100,0,500);
		TH1D* leading_pruned_jet_mass_a_evcut = new TH1D("leading_pruned_jet_mass_a_evcut"+zpmass[sample_i],"leading_pruned_jet_mass_a_evcut"+zpmass[sample_i],100,0,500);
		TH1D* leading2_pruned_jet_mass_b_evcut = new TH1D("leading2_pruned_jet_mass_b_evcut"+zpmass[sample_i],"leading2_pruned_jet_mass_b_evcut"+zpmass[sample_i],100,0,500);
		TH1D* leading2_pruned_jet_mass_a_evcut = new TH1D("leading2_pruned_jet_mass_a_evcut"+zpmass[sample_i],"leading2_pruned_jet_mass_a_evcut"+zpmass[sample_i],100,0,500);
		...
		// event loop : set up MC
	  	...
	  	for (Long64_t Nth = 0; Nth < Nev_MC; Nth++)
			...
			Int_t *genParSt=inner_data_for_MC.GetPtrInt("genParSt"); 
	      		...
	      		// FATjet var
	      		Int_t FATnJet=inner_data_for_MC.GetInt("FATnJet");
	      		
	      		TClonesArray* FATjetP4 = (TClonesArray*) inner_data_for_MC.GetPtrTObject("FATjetP4");
	      		Float_t *FATjetCSV=inner_data_for_MC.GetPtrFloat("FATjetCSV");
	      	      	Float_t *FATjetPRmass=inner_data_for_MC.GetPtrFloat("FATjetPRmass");
	      	      	Float_t *FATjetSDmass=inner_data_for_MC.GetPtrFloat("FATjetSDmass");
	      	      //	Float_t* FATjetPrunedEn = inner_data_for_MC.GetPtrFloat("FATjetPrunedEn");
	      	      //	Int_t* FATjetPassID = inner_data_for_MC.GetPtrInt("FATjetPassID");
	      	      	Float_t* FATjetTau1 = inner_data_for_MC.GetPtrFloat("FATjetTau1");
			Float_t* FATjetTau2 = inner_data_for_MC.GetPtrFloat("FATjetTau2");
			Float_t* FATjetCISVV2 = inner_data_for_MC.GetPtrFloat("FATjetCISVV2");
			
			
			vector<bool> &FATjetPassIDTight = *((vector<bool>*) inner_data_for_MC.GetPtr("FATjetPassIDTight"));			
			vector<bool> &FATjetPassIDLoose  = *((vector<bool>*) inner_data_for_MC.GetPtr("FATjetPassIDLoose"));

			vector<Float_t> *FATsubjetSDCSV=inner_data_for_MC.GetPtrVectorFloat("FATsubjetSDCSV");
	      	      		
/*      	      	
	  		Float_t *FATjetNHadEF=inner_data_for_MC.GetPtrFloat("FATjetNHadEF");
	  		Float_t *FATjetNEmEF=inner_data_for_MC.GetPtrFloat("FATjetNEmEF");
	  		Float_t *FATjetCHadEF=inner_data_for_MC.GetPtrFloat("FATjetCHadEF");
	  		Float_t *FATjetCEmEF=inner_data_for_MC.GetPtrFloat("FATjetCEmEF");
	  		Float_t *FATjetCMulti=inner_data_for_MC.GetPtrFloat("FATjetCMulti");
*/
...
*/
			...
			*/
			...
			{	
				//if (FATjetCSV[i]<0 || FATjetCSV[i]>1){continue;}
				if (FATjetCISVV2[i]<0 || FATjetCISVV2[i]>1){continue;}

								
				vector<FATsubjet> this_subj_vector;
				bool subjet_allright=true;
				if (FATsubjetSDCSV->size()!=2){continue;}
				for (int j=0;j<FATsubjetSDCSV->size();j++)
				{
					if(FATsubjetSDCSV[i][j]<0 || FATsubjetSDCSV[i][j]>1){subjet_allright=false; break; }
					FATsubjet this_subj(FATsubjetSDCSV[i][j]);
					this_subj_vector.push_back(this_subj);	 
				}
				if (!subjet_allright){continue;}
				
				FATjet jet_i( *((TLorentzVector*)FATjetP4->At(i)), FATjetCSV[i], FATjetCISVV2[i], FATjetPRmass[i],FATjetSDmass[i], FATjetTau1[i], FATjetTau2[i],FATjetPassIDTight[i],FATjetPassIDLoose[i]);
				this_ev.FATjets.push_back(jet_i);
				
				this_ev.FATsubjets.push_back(this_subj_vector);

			}
			for(int i=0;i<nGenPar;i++)  //gen par loop for Zprime_to_ZH sample
			{
			
				if (abs(genMomParId[i])!=23 && abs(genMomParId[i])!=25){continue;}
			...
			for (int i=0;i<this_ev.mus.size();i++)
			{
				...
				mu p = this_ev.mus[i];
				if ( !is_pass_mu_tight_id(p) ){continue;}
				this_ev.good_mus.push_back(p);	
			}
			
			// get FATjet after deoverlap with other sub jet and good lepton in every event
	     		for (Int_t i=0;i<this_ev.FATjets.size();i++)
			{
				FATjet this_j = this_ev.FATjets[i];
				...
				this_ev.FATjets_deoverlap_with_lepton.push_back(this_j);
		...
		int max_N_of_candidate_nbjet=0;
	  	for (Long64_t Nth = 0; Nth < Nev_MC; Nth++)
			...
			// matching gen par and FATjet

			if(!is_pass_ev_selection(events_MC[Nth])){continue;}

	 		//to check this event has 2 b-FATjet, to check deltaR of this 2 subjet > 0.3
			int N_of_FATjet=0;
			int N_of_candidate_bjet=0;
			int N_of_candidate_nbjet=0;
			double R=0.8;

			//vector< vector<FATsubjet> > this_FATsubjets=events_MC[Nth].FATsubjets;
			vector<FATjet>  this_FATjets=events_MC[Nth].FATjets_deoverlap_with_lepton;
			for (int i=0;i<this_FATjets.size();i++)
			{	
			
					FATjet this_j=this_FATjets[i];
			...
			N_of_candidate_nbjet=N_of_FATjet-N_of_candidate_bjet;
		...
		*/
		...
		// event loop : get fraction ratio matrix
	  	for (Long64_t Nth = 0; Nth < Nev_MC; Nth++)
			...
			// matching gen par and FATsubjet
			if(!is_pass_ev_selection(events_MC[Nth])){continue;}

	 		//to check this event has 2 b-FATsubjet, to check deltaR of this 2 subjet > 0.3
			int N_of_FATjet=0;
			int N_of_candidate_bjet=0;
			int N_of_candidate_nbjet=0;
		...
		// event loop : counting b-tagging for MC
	  	for (Long64_t Nth = 0; Nth < Nev_MC; Nth++)
			...
			for(int i=0;i<events_MC[Nth].FATjets_deoverlap_with_lepton.size();i++)
			{
				if(events_MC[Nth].FATjets_deoverlap_with_lepton[i].csv<0 || events_MC[Nth].FATjets_deoverlap_with_lepton[i].csv>1){continue;}
				FATCSV_b_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[i].csv);
			}

			if (events_MC[Nth].FATjets_deoverlap_with_lepton.size()>=1)
			{ leading_pruned_jet_mass_b_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[0].pruned_mass);}
			if (events_MC[Nth].FATjets_deoverlap_with_lepton.size()>=2)
			{ leading2_pruned_jet_mass_b_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[1].pruned_mass);}
			vector<gen_par> gen_b_from_H;
			for (int i=0;i<events_MC[Nth].gen_pars.size();i++)
			{
				gen_par this_g = events_MC[Nth].gen_pars[i];
				if(is_wanted_b(this_g)){gen_b_from_H.push_back(this_g);}
			}
			if (gen_b_from_H.size()==2)
			{genbb_deltaR_b_evcut->Fill(delta_R(gen_b_from_H[0], gen_b_from_H[1]) ); }
			...
			{
				if(events_MC[Nth].FATjets_deoverlap_with_lepton[i].csv<0 || events_MC[Nth].FATjets_deoverlap_with_lepton[i].csv>1){continue;}
				FATCSV_a_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[i].csv);
			}

			if (events_MC[Nth].FATjets_deoverlap_with_lepton.size()>=1)
			{ leading_pruned_jet_mass_a_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[0].pruned_mass);}
			if (events_MC[Nth].FATjets_deoverlap_with_lepton.size()>=2)
			{leading2_pruned_jet_mass_a_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[1].pruned_mass);}
			
			//counting
			for(int i=0;i<events_MC[Nth].FATjets.size();i++){ N_FATjet_before_deoverlap++;}
			for(int i=0;i<events_MC[Nth].FATjets_deoverlap_with_lepton.size();i++){ N_FATjet_after_deoverlap++;}		
			...
			//----------------------------k----------------------t-------------------------------
			//cout<<"this event have "<<N_FATsubjet_this_ev<<"jet, and "<<N_jet_pass_cut<< "jet pass cut, and "<<N_btagging<<" btagging."<<endl;
		...
		cout<<"<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< "<<samples[sample_i]<<" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"<<endl;	    	

		cout<<"N_available_event_MC="<<N_available_event_MC<<endl;
		cout<<"N_available_event_MC/Nev_MC ="<<double(N_available_event_MC)/double(Nev_MC)<<endl;
...
*/
		...
		zprime_sample_result.add_result(Nev_MC, N_available_event_MC, true_eff, true_mr, best_eff_rec, results_btagging2p_rec[0].err, best_mr_rec,results_btagging2p_rec[1].err );
...
/*	
		...
		cout<<"=================  counting # of FATjets before/after deoverlap with good lepton  ======================"<<endl<<endl;
		cout<<"N_FATjet_before_deoverlap:"<<N_FATjet_before_deoverlap<<endl;
		cout<<"N_FATjet_after_deoverlap:"<<N_FATjet_after_deoverlap<<endl;
		cout<<"fraction:"<<((double)N_FATjet_before_deoverlap-(double)N_FATjet_after_deoverlap)/(double)N_FATjet_before_deoverlap<<endl;
*/
	...
	/*
	  	...
	  	for (Long64_t Nth = 0; Nth < Nev_MC; Nth++)
			...
			Int_t *genMo1=inner_data_for_MC.GetPtrInt("genMo1");
	      		...
	      		// FATjet var
	      		Int_t FATnJet=inner_data_for_MC.GetInt("FATnJet");
	      		Float_t *FATjetPt=inner_data_for_MC.GetPtrFloat("FATjetPt");  
	      		Float_t *FATjetEta=inner_data_for_MC.GetPtrFloat("FATjetEta"); 
	      		Float_t *FATjetPhi=inner_data_for_MC.GetPtrFloat("FATjetPhi"); 
	     		Float_t *FATjetEn=inner_data_for_MC.GetPtrFloat("FATjetEn");
	     		Float_t *FATjetMass=inner_data_for_MC.GetPtrFloat("FATjetMass");
	      		Float_t *FATjetCSV=inner_data_for_MC.GetPtrFloat("FATjetCSV");
			...
			//cout<<"nGenJet:"<<nGenJet<<","<<"AK5nJet:"<<AK5nJet<<","<<"FATnJet:"<<FATnJet;
	...
	*/
	save_hist(FATCSV_b_evcut, zpmass[sample_i], "FATCSV_b_evcut");
	save_hist(FATCSV_a_evcut, zpmass[sample_i], "FATCSV_a_evcut" );	
	save_hist(genbb_deltaR_b_evcut, zpmass[sample_i], "genbb_deltaR_b_evcut" );
	save_hist(leading_pruned_jet_mass_b_evcut, zpmass[sample_i], "leading_pruned_jet_mass_b_evcut" );
	save_hist(leading_pruned_jet_mass_a_evcut, zpmass[sample_i], "leading_pruned_jet_mass_a_evcut" );	
	save_hist(leading2_pruned_jet_mass_b_evcut, zpmass[sample_i], "leading2_pruned_jet_mass_b_evcut" );
	save_hist(leading2_pruned_jet_mass_a_evcut, zpmass[sample_i], "leading2_pruned_jet_mass_a_evcut" );	
		...
		//Zee_IM->Write();
	/*
		FATCSV_b_evcut->Write(); 
		FATCSV_a_evcut->Write();
