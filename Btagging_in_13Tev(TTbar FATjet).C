...
bool is_pass_ev_selection_ttbar_semileptonic(event ev)
{
	...
	for (int i=0;i<ev.FATjets_deoverlap_with_lepton.size();i++)
	{
	
		FATjet this_j = ev.FATjets_deoverlap_with_lepton[i];
		
		if(!(this_j.p4.Pt()>30 && abs(this_j.p4.Eta()<2.5)  && /*this_j.sd_mass>150 && this_j.sd_mass<200 &&  this_j.tau2/this_j.tau1<0.5 && */ is_pass_FATjet_loose_id(this_j)))
		{continue;}
...
void Btagging_in_13Tev()
{
	...
	for (int sample_i=0;sample_i<1; sample_i++)
	{
		...
		//hist set
		TH1D* h_FATcisvv2_b_evcut = new TH1D("FATcisvv2_b_evcut","FATcisvv2_b_evcut",100,0,1);
		TH1D* h_FATcisvv2_a_evcut = new TH1D("FATcisvv2_a_evcut","FATcisvv2_a_evcut",100,0,1);

		TH1D* h_genbw_deltaR_b_evcut = new TH1D("genbw_deltaR_b_evcut","genbw_deltaR_b_evcut",100,0,3);		
		TH1D* h_FATnjet = new TH1D("FATnjet","FATnjet",6,0,6);
		TH1D* h_THINnjet = new TH1D("THINnjet","THINnjet",6,0,6);
		TH1D* h_FATnjet_deoverlap = new TH1D("FATnjet_deoverlap","FATnjet_deoverlap",6,0,6);
		
		TH1D* h_leading_jet_SDmass_b_evcut = new TH1D("leading_jet_SDmass_b_evcut","leading_jet_SDmass_b_evcut",100,0,500);
		TH1D* h_leading_jet_SDmass_a_evcut = new TH1D("leading_jet_SDmass_a_evcut","leading_jet_SDmass_a_evcut",100,0,500);
		TH1D* h_leading2_jet_SDmass_b_evcut = new TH1D("leading_jet2_SDmass_b_evcut","leading_jet2_SDmass_b_evcut",100,0,500);
		TH1D* h_leading2_jet_SDmass_a_evcut = new TH1D("leading_jet2_SDmass_a_evcut","leading_jet2_SDmass_a_evcut",100,0,500);
...
*/
			...
			*/
			...
			for(int i=0;i<nGenPar;i++)  //gen par loop for Zprime_to_ZH sample
			{
				...
				if (abs(genMomParId[i])!=6  && abs(genMomParId[i])!=24){continue;}
	     		...
	     		for (Int_t i=0;i<this_ev.FATjets.size();i++)
			{
				...
				{	
					...
					if (delta_R(this_j, this_ele)<0.8){overlap_with_lepton=true;break;}
				}

				for (int mui=0;mui<this_ev.good_mus.size();mui++)
				{
					mu this_mu = this_ev.good_mus[mui];
					if (delta_R(this_j, this_mu)<0.8){overlap_with_lepton=true;break;}
				}
		...
		*/
		...
		// event loop : counting b-tagging for MC
	  	for (Long64_t Nth = 0; Nth < Nev_MC; Nth++)
			...
			h_FATnjet->Fill(events_MC[Nth].FATjets.size());
			h_FATnjet_deoverlap->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton.size()); //note: dont include subjet deoverlap 
			for(int i=0;i<events_MC[Nth].FATjets_deoverlap_with_lepton.size();i++)
			{
				...
				h_FATcisvv2_b_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[i].cisvv2);
			}

			if (events_MC[Nth].FATjets_deoverlap_with_lepton.size()>=1)
			{ h_leading_jet_SDmass_b_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[0].sd_mass);}
			if (events_MC[Nth].FATjets_deoverlap_with_lepton.size()>=2)
			{ h_leading2_jet_SDmass_b_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[1].sd_mass);}
			...
			{
				if(events_MC[Nth].FATjets_deoverlap_with_lepton[i].csv<0 || events_MC[Nth].FATjets_deoverlap_with_lepton[i].csv>1){continue;}
				h_FATcisvv2_a_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[i].cisvv2);
			}

			if (events_MC[Nth].FATjets_deoverlap_with_lepton.size()>=1)
			{ h_leading_jet_SDmass_a_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[0].sd_mass);}
			if (events_MC[Nth].FATjets_deoverlap_with_lepton.size()>=2)
			{h_leading2_jet_SDmass_a_evcut->Fill(events_MC[Nth].FATjets_deoverlap_with_lepton[1].sd_mass);}
...
*/
	...
	*/
	...
	save_hist(h_FATcisvv2_b_evcut, "pic_13Tev_TTbar", "FATcisvv2_b_evcut");
	save_hist(h_FATcisvv2_a_evcut, "pic_13Tev_TTbar", "FATcisvv2_a_evcut");
	save_hist(h_genbw_deltaR_b_evcut, "pic_13Tev_TTbar", "genbw_deltaR_b_evcut");
	save_hist(h_FATnjet, "pic_13Tev_TTbar", "FATnjet");
	save_hist(h_FATnjet_deoverlap, "pic_13Tev_TTbar", "FATnjet_deoverlap");
	save_hist(h_leading_jet_SDmass_b_evcut, "pic_13Tev_TTbar", "leading_jet_SDmass_b_evcut");
	save_hist(h_leading_jet_SDmass_a_evcut, "pic_13Tev_TTbar", "leading_jet_SDmass_a_evcut");
	save_hist(h_leading2_jet_SDmass_b_evcut, "pic_13Tev_TTbar", "leading2_jet_SDmass_b_evcut");
	save_hist(h_leading2_jet_SDmass_a_evcut, "pic_13Tev_TTbar", "leading2_jet_SDmass_a_evcut");
